...
Blue Zones are specified by **KTP-Zones**: the gradient of environments from
Deep Blue to Wild, what a zone operator guarantees at each step of it, how a
zone advertises its type so an agent knows the rules before it connects, and
what happens to trust at a border between zones.
