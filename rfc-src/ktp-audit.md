...
1. Record Everything: Every authorization decision, not just denials or anomalies. The baseline is as important as the exceptions.

1. Context Over Outcome: The derived decision verb is one bit's worth of reading. The context (why, what was the environment, what would have happened) is where the value lies.

1. Immutability: Once written, records cannot be altered or deleted. This is essential for legal defensibility and forensic integrity.

1. Queryability: Data that can't be queried is data that won't be used. The Flight Recorder must support rich queries for forensic analysis.

1. Forward Secrecy: Records should be usable for learning without exposing sensitive operational details.
...
1. Hash algorithm: SHA-256 (minimum)

1. Chain creates tamper-evident sequence

1. Any modification breaks the chain
...
1. Alert on any hash mismatch or sequence gap

1. Maintain chain anchors in external system (e.g., public blockchain timestamp)

1. Support third-party audit of chain integrity

Tamper detection response:

1. Immediately alert security team

1. Preserve evidence (snapshot of tampered section)

1. Isolate affected storage

1. Begin forensic investigation
...
1. Human negligence: A human made a decision that violated policy or caused harm through carelessness.

1. Environmental force majeure: The system correctly responded to environmental conditions beyond anyone's control.
...
1. Trust prediction: Predict E_trust trajectory from current conditions

1. Risk calibration: Learn optimal A values from historical outcomes

1. Sensor validation: Detect sensor drift or manipulation
